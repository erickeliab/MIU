...
public class Problem1 {
    ...
    public static List<String> elementsInBoth(List<String> list1, List<String> list2) {
        //implement
//
        return list2.stream().filter(list1::contains).toList();
    ...
    public static List<String> getZipsOfSpecialCustomers(List<Customer> list) {
        //implement
        return list.stream().filter(e -> (e.getCity().length() >= 6 && !e.getCity().contains("e"))).map(Customer::getZip).distinct().sorted().toList();
